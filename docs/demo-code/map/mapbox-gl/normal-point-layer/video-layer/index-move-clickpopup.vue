...
import MovePopup from './move-popup';
import api from './api';
...
export default {
    ...
    methods: {
        ...
        async drawLayer() {
            ...
            if (this.map.getSource(this.sourceName)) {
                ...
                // 悬浮弹窗
                this.movePopup = new mapboxgl.Popup({
                    offset: {
                        top: [0, 0],
                        bottom: [0, -30],
                    },
                ...
                // 弹窗关闭时，需要手动销毁vue组件实例
                this.movePopup.on('close', () => {
                    // 清除高亮
                    this.movePopupVm?.$destroy?.();
                    this.movePopupVm = null;
                });

                // 鼠标移过去，变成手型
                this.map.on('mouseover', this.layerName, this.onMouseEnterHandler);

                // 鼠标离开，形状恢复
                this.map.on('mouseout', this.layerName, this.onMouseLeaveHandler);
        ...
        onMouseEnterHandler(event) {
            ...
            //判断当前选中的是空白区域还是点位
            if (features[0]) {
                let mapData = features[0].properties;
                
				this.$nextTick(() => {
					this.openMovePopup(mapData);
				})
            }
        },
        //鼠标移出事件
        ...
        openMovePopup(data) {
            ...
            this.movePopup.remove();
            this.movePopup
                .setHTML(`<div id='move-popup'></div>`)
                .setLngLat(lngLat)
                .addTo(this.map)

            this.movePopupVm = new Vue({
                render(h) {
                    return h(MovePopup, {
                        props: {
                            postData: data,
            ...
            this.movePopupVm.$mount('#move-popup');
        },
        onClickHandler(e) {
    ...
    beforeDestroy() {
        ...
        this.map.off('mouseenter', this.layerName, this.onMouseEnterHandler);
        this.map.off('mouseleave', this.layerName, this.onMouseLeaveHandler);
